...
<template>
    <AppLayout>
        <div class="page">
            <div class="page-head">
                <div>
                    <h1 class="page-head__title">Editar rol</h1>
                    <p class="page-head__sub">Modifica los permisos de <strong>{{ role.name }}</strong>.</p>
                </div>
            ...
            <form @submit.prevent="submit" class="card" novalidate>
                ...
                <div class="section">
                    ...
                    <p v-if="systemRoles.includes(role.name)" class="hint">
...
.perms-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: var(--space-2); }
...
.perm-check__box { width: 16px; height: 16px; border: 2px solid var(--color-border); border-radius: var(--radius-sm); flex-shrink: 0; background: var(--color-surface); transition: border-color var(--transition-interactive), background var(--transition-interactive); position: relative; }
.perm-check input:checked ~ .perm-check__box { background: var(--color-primary); border-color: var(--color-primary); }
.perm-check input:checked ~ .perm-check__box::after { content: ''; position: absolute; left: 3px; top: 1px; width: 6px; height: 9px; border: 2px solid white; border-top: none; border-left: none; transform: rotate(45deg); }
